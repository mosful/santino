  ...
  return (
    <div className="space-y-4 text-sm">
      ...
      <div className="rounded-xl border border-slate-200 p-3">
        <div className="mb-2 text-xs font-medium text-slate-600">＋ 新增片語</div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          className="w-56 rounded-lg border border-slate-200 px-2 py-1.5 text-xs"
        />
        <span className="ml-auto text-xs text-slate-400">共 {filtered.length} 則</span>
      </div>

      {allTags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 text-xs">
          <span className="text-slate-400">🏷️ 現有標籤：</span>
          {allTags.map((t) => (
      ...
      <ul className="space-y-2">
        ...
        {filtered.length === 0 && (
          <li className="rounded-xl border border-dashed border-slate-300 p-6 text-center text-xs text-slate-400">
            查無片語
          </li>
        )}
      </ul>
